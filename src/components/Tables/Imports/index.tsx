  ...
  import CreateExcelModal from "./CreatedExcelModal";
  // Dummy user data
          ...
          <div className="flex w-full flex-col gap-4 md:flex-row md:items-center md:gap-3">
            ...
            <div className="relative">
              <input
                ...
                className="w-full cursor-pointer rounded-md border border-gray-300 p-3 text-sm dark:bg-gray-dark dark:text-white md:w-auto"
              />

              {showDatePicker && (
                <div className="absolute z-50 mt-2 rounded-md border  bg-white p-2 shadow-lg dark:bg-gray-dark">
                  <DateRange
                    ranges={range}
                    onChange={(item: any) => {
                      setRange([item.selection]);
            ...
            {loading ? (
              ...
              excels.map((user: any) => (
                <TableRow key={user._id}>
                  <TableCell className="text-left min-h-2 max-h-2">
                    {user.transactionid}
                  </TableCell>
